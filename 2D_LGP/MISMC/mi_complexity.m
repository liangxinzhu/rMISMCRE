...
%                 4*exp(log(2)*s*sqrt(2))*(temp^2+temp) ));
            L = max(ceil(log((log(1/eps))^2/eps)/log(2)/2/s), 2*K+1);
            for al = K:L
                for bl = K:L-al
            ...
            var   = 2.^(-beta*L_ma);
            var(1,1) = 1e-3;
            cost  = L_ma.*2.^L_ma;%L_ma.*2.^L_ma;%2.^L_ma;
            ...
            parfor j = 1:M
                ...
                for lx = K:L
                    for ly = K:L
                        ...
                        [sums, cst] = mismc_l(lx,ly,N,data,rate,a,b,mu,K);
                        sumlsn  = sumlsn  + sums(1);
                        sumlret = sumlret + sums(2);%/N
                        sumlreb = sumlreb + sums(3);%/N
                        costl   = costl + N*cst;
                    end
                end
                misn(j,i) = sumlsn;
