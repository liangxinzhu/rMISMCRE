% ----------------------------------------------------------------- %
% function [sums, cost] = opre_rmismc_l(lx,ly,N,data,rate,a,b,mu,k)
%
...
for i = 1:N
    % sampling from prior
    if lx == k && ly == k
        [prior1] = LGprior(lx,ly,rate,a,b,mu,1);
        fun1 = interpob(prior1,Nx,Ny,xpos,ypos);
        llik1 = sum(fun1(:)-mu) - n*log(sum(exp(prior1(:)))/Nx/Ny);
        f1(i) = llik1;
        u1(i,:) = prior1(:);
        G(i) = llik1; 
    elseif ly == k && lx > k
        [prior1,prior2] = LGprior(lx,ly,rate,a,b,mu,2);
        
        fun1 = interpob(prior1,Nx,Ny,xpos,ypos);
        fun2 = interpob(prior2,Nx/2,Ny,xpos,ypos);
        
        llik1  = sum(fun1(:)-mu) - n*log(sum(exp(prior1(:)))/Nx/Ny);
        llik2  = sum(fun2(:)-mu) - n*log(sum(exp(prior2(:)))/Nx/Ny*2);
    ...
    elseif lx == k && ly > k
        [prior1,prior3] = LGprior(lx,ly,rate,a,b,mu,3);
        
        fun1 = interpob(prior1,Nx,Ny,xpos,ypos);
        fun3 = interpob(prior3,Nx,Ny/2,xpos,ypos);
        
        llik1 = sum(fun1(:)-mu) - n*log(sum(exp(prior1(:)))/Nx/Ny);
        llik3 = sum(fun3(:)-mu) - n*log(sum(exp(prior3(:)))/Nx/Ny*2);
        ...
        [prior1,prior2,prior3,prior4] = LGprior(lx,ly,rate,a,b,mu,4);
        ...
        llik2  = sum(fun2(:)-mu) - n*log(sum(exp(prior2(:)))/Nx/Ny*2);
        llik3  = sum(fun3(:)-mu) - n*log(sum(exp(prior3(:)))/Nx/Ny*2);
        llik4  = sum(fun4(:)-mu) - n*log(sum(exp(prior4(:)))/Nx/Ny*4); 
...
sums(2) = exp(Z)*( reinc1 - reinc2 - reinc3 + reinc4 );
sums(3) = exp(Z)*( reinc11 - reinc12 - reinc13 + reinc14);
